...
**Step 1: Read Browser Configuration from karma.conf.js**

**MANDATORY:** Read `karma.conf.js` to identify the configured browser:
```javascript
read_file('karma.conf.js', 1, 100)
// Look for the browsers array, e.g.:
// browsers: ['EdgeHeadless']
// browsers: ['ChromeHeadless']
// browsers: ['Edge']
```

Extract the browser name from the `browsers` array. Common values:
- `EdgeHeadless` or `Edge` → Use EdgeHeadless
- `ChromeHeadless` or `Chrome` → Use ChromeHeadless
- If array is empty or undefined → Use default (no --browsers flag)

**Step 2: Run Tests with Detected Browser**

```bash
# Example 1: If karma.conf.js has browsers: ['EdgeHeadless']
npm test -- --code-coverage --watch=false --browsers=EdgeHeadless

# Example 2: If karma.conf.js has browsers: ['ChromeHeadless']
npm test -- --code-coverage --watch=false --browsers=ChromeHeadless

# Example 3: If karma.conf.js uses default or no specific browser
npm test -- --code-coverage --watch=false
```

**CRITICAL:** Always use the browser found in karma.conf.js, don't assume ChromeHeadless.
...
```bash
# Use browser from karma.conf.js (EdgeHeadless, ChromeHeadless, or default)
npm test -- --code-coverage --watch=false
# OR with explicit browser if needed:
npm test -- --code-coverage --watch=false --browsers=EdgeHeadless
```
...
```bash
# Use browser configuration from karma.conf.js
npm test -- --code-coverage --watch=false
...
```bash
# Use browser configuration from karma.conf.js (EdgeHeadless/ChromeHeadless)
npm test -- --code-coverage --watch=false
npm test -- --code-coverage --watch=false
npm test -- --code-coverage --watch=false
```
...
**Browser:** [Check karma.conf.js - EdgeHeadless/ChromeHeadless/etc]
...
1. **Check `karma.conf.js`** to identify browser configuration (Edge, EdgeHeadless, ChromeHeadless, etc.)
2. **Execute PHASE 0** (run tests to get current state using configured browser)
3. **CREATE `coverage-baseline-report.md`** ← MANDATORY FIRST DELIVERABLE
4. **Based on test results:**
   - If failures > 0: Execute PHASE 1 (fix all failures)
   - If failures = 0: Execute PHASE 2 (gap analysis)
5. **CREATE `coverage-gap-analysis.md`** ← MANDATORY SECOND DELIVERABLE
6. **Execute PHASE 3-5** (systematic test generation until 95% achieved)
7. **CREATE `coverage-final-report.md`** ← MANDATORY FINAL DELIVERABLE
8. **Update todo list after each phase** to track progress
9. **Report completion** with final coverage metrics in tabular format
